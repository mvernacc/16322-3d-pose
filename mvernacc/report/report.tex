...
\begin{equation}
  \vec{a}_{meas}^{sensor}(t) = \textproc{RotateFrame}(\vec{g}_{Earth}^{NED}, q_{NED2sensor}(t)) + \vec{\nu}_{accel}(t)
\end{equation}
where:
\begin{itemize}
...
\end{equation}
The measurement noise standard deviation is typically specified as a continuous-time Fourier spectral density $\sigma_{accel,c,*}$, with units of \si{\meter\per\second\squared\per\sqrt\second}. This can be converted to discrete time via:
\begin{equation}
  \sigma_{accel,d,*} = \sigma_{accel,c,*} \sqrt{\Delta t}
\end{equation}
